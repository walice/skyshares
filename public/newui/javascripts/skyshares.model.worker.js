...
var model = {
			...
			skyshares.rest.get( 'http://skyshares-soda.rhcloud.com/mac/' + mac_year, {
					onloadend : function(evt) {
						...
						if ( data ) {
							...
							self.mac_data.members.sort(comparemacyear);
							self.download_queue.remove( 'mac_' + data.year );
							//
							// JONS: debugging
							//
							self.mac_data.members.forEach( function( mac ) {
								log( mac.name );
							});
	...
	run : function() {
		...
		try {
			...
			// calculate EQ price
			//
			log( 'calculating EQ price curve' );
			...
			for (var yr=2010; yr <=2100; yr++) {
				...
				self.cow_countries.forEach( function( country ) { // for each iCOW
					...
					if ( trading_scenario == self.getdata( 'endogenous_regulation' ).value ) {
						var r = regul( country.iso_index, yr );
						if ( abat( country.iso_index, yr ) > 0 ) {
							abatement_target_bis += abat( country.iso_index, yr )*((100-r)/100);
						}
					} else {
	...
	runtime_functions : {
		...
		wpEQ : function ( t ) {
			...
			/* JONS : pre and fin now include all years
			for ( var i = 0; i <= self.EQPrice.length - 1; i++ ) {
