...
skyshares.admin = {
  init: function () {
    ...
    // populate data lists
    //
    ;['constant', 'group', 'dataset', 'function', 'variable'].forEach(function (
      data_type
    ) {
  ...
  add: function (type) {
    var win = window.open(
      '/editor/new/' + type,
      'skyshares.share',
      'width=640,height=480,modal=yes'
    )
    if (win) {
  ...
  deleteData: function (item) {
    if (
      confirm(
        'Are you sure you want to remove ' + item.name + ' from the database'
      )
    ) {
