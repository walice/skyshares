...
skyshares.editor = {
		...
		if ( fileInput ) {
			...
			skyshares.filereader.readfile( fileInput, types, {
				...
				onerror : function( evt ) {
					alert( 'Error reading data : ' + skyshares.filereader.errordescription( evt.target.error ) );
				}
			} );
		}
	},
			...
			skyshares.rest.put( url, json, {
				onloadend : function( e ) {
					...
					if ( request.status === 200 ) {
						...
						alert( 'Failed to save ' + json.name + ' : ' + request.status + ' : ' + request.statusText );
					}
				},
				onerror : function( e ) {
			...
			skyshares.rest.post( '/data', json, {
				onloadend : function( e ) {
					...
					if ( request.status === 200 ) {
						...
						alert( 'Failed to add ' + json.name + ' : ' + request.status + ' : ' + request.statusText );
					}
				},
				onerror : function( e ) {
					var message = e.target;
	...
	evaluatefunction : function() {
		...
		var compiled_function = mathjsfunction.scope[ mathjsfunction.function_name ].bind( mathjsfunction );
		//
		//
		//
		...
		for ( var i = 1; i < mathjsfunction.parameters.length; i++ ) {
			...
			p.innerHTML += ' min <input id="' + mathjsfunction.parameters[ i ] + '-min" type="number" value="0" />';
			p.innerHTML += ' max <input id="' + mathjsfunction.parameters[ i ] + '-max" type="number" value="0" />';
			p.innerHTML += ' step <input id="' + mathjsfunction.parameters[ i ] + '-step" type="number" value="1.0" />';
			evaluation.appendChild( p );
		}
		...
		graph.onclick = function( e ) {
			try {
				...
				for ( var i = 1; i < mathjsfunction.parameters.length; i++ ) {
					min_max.push( {
						min : parseFloat( document.getElementById( mathjsfunction.parameters[ i ] + '-min' ).value ),
						max : parseFloat( document.getElementById( mathjsfunction.parameters[ i ] + '-max' ).value ),
						step : parseFloat( document.getElementById( mathjsfunction.parameters[ i ] + '-step' ).value )
					} );
				}
				//
				...
				var range = max - min;
				if ( min_max.length == 1 ) {
					for ( var i = min_max[ 0 ].min; i <= min_max[ 0 ].max; i += min_max[ 0 ].step ) {
					...
					for ( var i = min_max[ 0 ].min; i <= min_max[ 0 ].max; i += min_max[ 0 ].step ) {
						values.push( [] );
						for ( var t = min_max[ 1 ].min; t <= min_max[ 1 ].max; t += min_max[ 1 ].step ) {
							var value = mathjsfunction.scope[ mathjsfunction.function_name ]( i, t );
							if ( value < min ) min = value;
							if ( value > max ) max = value;
					...
					content.innerHTML = 'Unable to evaluate at present, please check back later';
					return;
				}
				range = max - min;
				//
				...
				context.lineTo( render_rect.x + render_rect.width, render_rect.y + render_rect.height );
				context.stroke();
				...
				if ( min_max.length == 1 ) { 
					...
					for ( var t = 0; t < values.length; t++ ) {
						...
						var y = render_rect.y + ( render_rect.height - ( render_rect.height * ( values[ t ] - min ) / range ) );
						if ( t == 0 ) {
							context.moveTo( x, y );
						} else {
							context.lineTo( x, y );
					...
					for ( var i = 0; i < values.length; i++ ) {
						...
						for ( var t = 0; t < values[ i ].length; t++ ) {
							...
							var y = render_rect.y + ( render_rect.height - ( render_rect.height * ( values[ i ][ t ] - min ) / range ) );
							if ( t == 0 ) {
								context.moveTo( x, y );
							} else {
								context.lineTo( x, y );
		...
		table.onclick = function( e ) {
			try {
				...
				if ( min_max.length == 0 ) {
						content.innerHTML += mathjsfunction.function_name + '() = ' + mathjsfunction.scope[ mathjsfunction.function_name ]() + '<br />';
				} else if ( min_max.length == 1 ) {
					for ( var i = min_max[ 0 ].min; i <= min_max[ 0 ].max; i += min_max[ 0 ].step ) {
						content.innerHTML += mathjsfunction.function_name + '( ' + i + ' ) = ' + mathjsfunction.scope[ mathjsfunction.function_name ]( i ) + '<br />';
					}
				} else if ( min_max.length == 2 ) {
					for ( var i = min_max[ 0 ].min; i <= min_max[ 0 ].max; i += min_max[ 0 ].step ) {
						for ( var t = min_max[ 1 ].min; t <= min_max[ 1 ].max; t += min_max[ 1 ].step ) {
							content.innerHTML += mathjsfunction.function_name + '( ' + i + ', ' + t + ' ) = ' + mathjsfunction.scope[ mathjsfunction.function_name ]( i, t ) + '<br />';
						}
					}
				} else {
					content.innerHTML = 'Unable to evaluate at present, please check back later';
