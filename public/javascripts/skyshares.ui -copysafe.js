		...
		function ui() {
			...
			*/
			...
			self.trading_scenario.addEventListener( 'load', function( evt ) {
				self.variables.regulated_share = ( 100.0 - self.trading_scenario.value );
				if (self.trading_scenario.value ==100 ){
					self.variables.trading_scenario == 0;
				} else {
					self.variables.trading_scenario == 2;
				}
			}, false );
			*/
			self.trading_scenario.addEventListener( 'change', function( evt ) {
				self.variables.regulated_share = ( 100.0 - self.trading_scenario.value );
				switch ( self.trading_scenario.value ) {
					case 100 : 
			...
			/*
				var current = trading_scenario.value;
				console.log('current var' + current);
				//self.variables.trading_scenario = 0;
				if ( current != 100 ){
					self.variables.trading_scenario == 2;
					self.model.postMessage( { command: 'setvariables', parameter: self.variables } );				
					...
					self.variables.trading_scenario == 0;
					console.log( 'trading_scenario changed to: ' + self.variables.trading_scenario );
					console.log( 'regulated_share changed to: ' + self.trading_scenario.value );
					self.model.postMessage( { command: 'setvariables', parameter: self.variables } );				
				}
			}, false );
			*/
			...
			//self.trading_scenario.setValue(self.variables.trading_scenario);
				...
				switch ( evt.data.command ) {
					...
					case 'update_country' :
						...
						if ( country ) { // TODO: encapsulate these properties in skyshares object
							...
							self.map.countries[ country.iso ].emissionscapita		= country.emissionscapita;
							self.map.countries[ country.iso ].allowancescapita		= country.allowancescapita;	
							self.updateranges();
